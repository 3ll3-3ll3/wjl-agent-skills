...
- Skill 默认位于 `skills/<skill-name>/`；同一主题下存在多个相关 Skill 时，可按分类放在 `skills/<category>/<skill-name>/`。
...
6. 新的个人 Skill 默认添加到 `skills/<skill-name>/`；同一主题形成一组 Skill 时，可统一归入 `skills/<category>/<skill-name>/`，而不是新建独立仓库。
...
当用户明确要求把现成第三方 Skill 原样引入本仓库时，为了保证来源可核对、行为不被悄悄改写，并方便后续与上游同步，允许该第三方 Skill 的上游文件保留原始语言。

适用条件：

- 仅限第三方上游原样镜像文件；本仓库自行编写或实质性改写的说明仍遵守简体中文主体规则。
- 每个第三方镜像 Skill 必须包含中文 `UPSTREAM.md`，记录上游仓库、原始路径、固定 commit、许可证与本地化状态；第三方 Skill 组可统一放在分类目录下。
- 未经用户明确要求，不把第三方 `SKILL.md` 翻译、压缩、重写或改造成另一套行为。
- 上游 Skill 依赖的模板、脚本或资源若被 `SKILL.md` 直接引用，应一并保留；无实际内容的占位目录可以省略。
- 第三方许可证或许可证说明必须随镜像保留或在 `UPSTREAM.md` 中明确指向。
