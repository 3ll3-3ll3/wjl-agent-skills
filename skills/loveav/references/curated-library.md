...
│  └─ seen-index.csv            # 旧库或历史输入中已见身份，可含待复核项
├─ exports/
└─ backups/
```
...
- `status` 建议使用 `active`、`removed`、`review`；移除后保留墓碑记录和原因。
...
`reference-blacklist.csv` 只取消参考女优标签的命中资格，不能删除精选番号。

`raindrop-export-blacklist.csv` 命中时只阻止该结果进入 Raindrop 导出，同时保留记录和排除原因。

如果需要屏蔽具体番号或链接，另建 `exact-exclude.csv`，不要伪装成女优标签黑名单。
