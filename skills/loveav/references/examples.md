...
```text
运行 LoveAV 全部功能，处理所有未读。
```

预期行为：运行所有非静默功能；不读取、不处理 Twitter 和海角，也不改变这两个来源的已读状态。

```text
运行 LoveAV 全部功能，这次包括海角。
```

预期行为：在非静默功能之外加入海角；Twitter 仍保持静默。
...
```text
使用 LoveAV，我要开始 MissAV 工作。
```

预期行为：

1. 先让用户选择“全部 av 分类群组未读”“番号待提取手动范围”“上传/粘贴”或组合来源。
2. 只有选择 `番号待提取` 时才继续询问消息链接/ID、日期或最近 N 条。
3. 再让用户选择标准处理、全量重查或只预览；标准处理排除正式主体库已有番号并生成一个合并脚本。
4. 明确展示全部 `av` 分类来源成功后按冻结上界标已读，手动范围默认不标已读。
5. 若用户一开始已经给齐来源、范围和处理要求，不重复走向导。
...
预期行为：直接检查 TG Exporter 中全部 `av` 分类候选的当前未读，排除主体库历史，生成一个合并脚本；不再询问已有默认值。
