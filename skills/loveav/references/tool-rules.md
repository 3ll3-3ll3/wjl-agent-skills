...
- 浏览器脚本必须基于 Skill 内已经验证的 v0.5.13 原版模板生成。数据注入只包括规范化后的番号文本、从正式 `missav-library.csv` 实时派生并应用第一层黑名单后的参考女优 Tag，以及第二层 Raindrop 导出黑名单；生成器可应用 `missav-browser-script.md` 明确定义并经测试的等价性能补丁。
...
- 注入脚本的参考女优 Tag 不再来自书签 HTML 或独立 Tag TXT。生成脚本时扫描正式主体库主 Tags 和全部来源变体，从每组 Tags 开头读取女优名，遇到系统 Tag、类型边界 Tag 或非女优形状即停止；中文、日文和合法拉丁字母姓名均可识别。该派生集合不单独落盘，也不反向修改主体库。浏览器选择和生成的“当前女优 Tag 合集”是另一个概念：它只作为已处理番号和女优映射的运行检查点。
