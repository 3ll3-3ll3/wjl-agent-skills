...
Raindrop 只作为搜索和浏览入口。不得把 Raindrop 导出 CSV 自动回灌、比较或合并进本地主库。默认模式从该频道当前可访问的完整历史重新生成主库；用户明确选择“当前未读增量”时，只把冻结范围中解析到的资源合并进既有主库，不能把本轮没有出现的旧资源判定为删除。两种模式都用固定 `current` 保存最新状态，`updates` 保存每次差异，数据变化前在 `snapshots` 保存上一版。
...
用户明确要求处理所有功能的当前未读，或明确要求功能 8 使用未读增量时，必须：

1. 分别冻结每个启用来源的当前未读范围；
2. 完整读取冻结范围；
3. 按规范 URL 合并进该来源 `current/resource-library.jsonl`，同一 URL 保留新旧全部来源消息；
4. 原子写入主库、增量、清单和必要快照；本轮未出现的旧资源保留不动；
5. 写入和校验全部成功后，才把该来源确认已读到冻结 `upper`；失败来源和快照后新消息保持未读。

未读增量模式的检查点 `strategy` 必须为 `incremental_unread_merge`。它与默认的 `full_rescan_with_incremental_outputs` 明确区分，不能拿增量批次伪装成全历史完整重建。
