...
用户可能提供学生邮箱、教师邮箱、邮箱域名、学校名、学院名或其他明显学校线索。Agent 自动识别学校，并根据明确的角色或域名证据选择学生或教师模式；确实无法判断时，只询问认证类型，不猜测。
...
教师认证与学生认证的学校查询、随机姓名和数字 ID、PPT 格式保护、渲染验收、聊天字段、重做和自动归档要求完全一致。教师模式只是在模板内部把同一个随机数字 ID 写入 `{{facultyid}}`。
...
两种模式都只替换当前模板允许的占位符，并保护非占位符格式和内容。

第一行姓名 + 数字 ID 必须单行；后续正文必须自然顺排；右下角学校官方英文全名必须单行；源模板中存在的演示或无效标识必须保留；交付前必须实际渲染 PNG 检查。

若替换后出现“一行一个单词”、字段或单词孤立成行，或后续文字没有按正常英文段落依次向后顺延，则直接判定验收失败。正确效果是后续单词按原段落顺序连续流动。只能做最小必要的文字流调整，禁止人为硬换行、硬拆词或用学校简称解决。

## 语言规范

本 Skill 的 `SKILL.md`、`README.md`、`AGENTS.md`、`docs/` 和其他说明文档统一使用简体中文。

只有占位符、字段名、路径、命令、文件名、代码以及 Google Drive、PPTX 等必须保持原样的技术名称保留英文。除此之外，不再维护英文正文或英文 README 副本。
